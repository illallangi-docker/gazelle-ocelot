...
void config::init() {
        ...
        // MySQL
        add("mysql_db", "{{MYSQL_DATABASE}}");
        add("mysql_host", "{{MYSQL_HOST}}:{{MYSQL_PORT}}");
        add("mysql_username", "{{MYSQL_USER}}");
        add("mysql_password", "{{MYSQL_PASSWORD}}");
        
        // Site communication
        add("site_host", "{{GAZELLE_FQDN}}");
        add("site_path", "");
        add("site_password", "{{OCELOT_SECRET}}");
        add("report_password", "{{OCELOT_REPORT_KEY}}");
